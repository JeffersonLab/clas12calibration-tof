...
public class CTOFCalibration
	...
	// 200, 0.0, 200.0);
	// public static H1F trackRCS2 = new H1F("red_chi_sq2","Reduced chi^2 for
	// tracks",
	// 100, 0.0, 40.0);
	...
	public CTOFCalibration() {
		...
		GStyle.getH1FAttributes().setOptStat("1111111111");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().compareTo("Finish") == 0) {
			...
			System.out.println(
					"Path length normalisation for gmean?: " + pathNormList.getItemAt(TOFCalibration.pathNorm));
			...
			System.out
					.println("Mass assumption for beta calculation: " + massAssList.getItemAt(TOFCalibration.massAss));
	...
	public void writeTimeOffsets(String filename, int[] stepOptions) {
		...
		try {
			...
			for (int paddle = 1; paddle <= CTOFCalibrationEngine.NUM_PADDLES[0]; paddle++) {
				...
				line = 1 + " " + 1 + " " + paddle + " " + new DecimalFormat("0.000").format(upDown) + " "
						+ new DecimalFormat("0.000").format(rfpad) + " " + new DecimalFormat("0.000").format(p2p);
				outputBw.write(line);
				outputBw.newLine();
			}
